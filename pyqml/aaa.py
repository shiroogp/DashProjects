...
#    socktemp = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # UDP
#    socktemp.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
#    socktemp.bind((ip,0))
   
#    allsockets.append(socktemp)
...
#send to Arduino
 #read echo back from arduino to verify receipt of message
   ...
   try:   
      ...
      # for socks in allsockets:
      #    try:
      #       socks.sendto(("155,"+vars[4]).encode('utf-8'), (UDP_IP, UDP_PORT))
      #       socks.sendto(("210,"+vars[24]).encode('utf-8'), (UDP_IP, UDP_PORT))
      #       socks.sendto(("135,"+vars[6]).encode('utf-8'), (UDP_IP, UDP_PORT))
      #       socks.sendto(("179,"+vars[14]).encode('utf-8'), (UDP_IP, UDP_PORT))
      #       socks.sendto(("6,"+vars[10]).encode('utf-8'), (UDP_IP, UDP_PORT))
      #       socks.sendto(("22,"+str(map_psi)).encode('utf-8'), (UDP_IP, UDP_PORT))
      #    except:
      #       print("Error sending using - ")
