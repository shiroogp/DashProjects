  ...
  //    ui(new Ui::MainWindow)
{
    ...
    foreach (QHostAddress address, QNetworkInterface::allAddresses()) {
        if (address.protocol() == QAbstractSocket::IPv4Protocol &&
            ...
            ipaddresses.push_back(address.toString());
        }
    }

    if (engine == Q_NULLPTR)
        return;
    engine->rootContext()->setContextProperty("SenderModel", mSenderModel);
    engine->rootContext()->setContextProperty("ReceiverModel", mReceiverModel);
    engine->rootContext()->setContextProperty("TransferServer", mTransServer);
...
MainWindow::MainWindow(QQmlApplicationEngine *mengine, DashBoard *dashboard, QObject *parent) :
    QObject(parent)
  , m_dashboard(dashboard)
{
    ...
    mReceiverModel = new TransferTableModel(this);
    mDeviceModel = new DeviceListModel(mBroadcaster, this);
    mTransServer = new TransferServer(m_dashboard, mDeviceModel, this);
    mTransServer->listen();
    //    ReceiverSelectorDialog dialog(mDeviceModel);
    mDeviceModel->refresh();
...
QStringList MainWindow::getmynetwork()
{

    mynetwork.clear();
    QVector<Device> receivers = mDeviceModel->getDevices();
    for (const Device& address : receivers) {
        if (address.isValid()) {
        mynetwork.push_back(address.getName() + " - " + address.getOSName() + " - " + address.getAddress().toString());
        }
    }
    return mynetwork;
}
QStringList MainWindow::getAddresses()
{
    return ipaddresses;
}
...
void MainWindow::onSendFilesActionTriggered(const QString &oldfileName)
{
    ...
    pairs.push_back( QPair<QString, QString>("", oldfileName) );
