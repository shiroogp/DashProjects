...
TransferServer::TransferServer(DashBoard *engine, DeviceListModel* devList, QObject *parent) : QObject(parent), mengine(engine)
{
    mDevList = devList;
...
void TransferServer::onNewConnection()
{
    ...
    if (socket) {
        ...
        Receiver* rec = new Receiver(mengine, dev, socket);
