...
void DashBoard::setNewDashFile(const QString &newDashFile)
{
    m_newdashfile = newDashFile;
    emit newDashFileChanged(newDashFile);
}
...
void DashBoard::setOrientation(const qreal &orientation)
{
...
void DashBoard::setdashsetup5(const QStringList &dashsetup5)
{
//    if (m_dashsetup4 == dashsetup4)
//        return;
    m_dashsetup5 = dashsetup5;
    emit dashsetup5Changed(dashsetup5);
}

void DashBoard::setdashsetup4(const QStringList &dashsetup4)
{
//    if (m_dashsetup4 == dashsetup4)
//        return;
    m_dashsetup4 = dashsetup4;
    emit dashsetup4Changed(dashsetup4);
}
...
void DashBoard::pageNo(const qreal &pageNo)
{
    if (m_pageNo == pageNo)
        return;
    m_pageNo = pageNo;
    emit pageNoChanged(pageNo);
}

void DashBoard::updatePageNo(const qreal &pageNo)
{
    if (m_pageNo == pageNo)
        return;
    m_pageNo = pageNo;
...
void DashBoard::sourceIndex(const qreal &sourceIndex)
{
    if (m_sourceIndex == sourceIndex)
        return;
    m_sourceIndex = sourceIndex;
    emit sourceIndexChanged(sourceIndex);
}
...
void DashBoard::setDone()
{
    emit done();
}
void DashBoard::setErrorOcurred(const QString& errStr)
{
    emit errorOcurred(errStr);
}
void DashBoard::setProgressChanged(int progress)
{
    emit progressChanged(progress);
}
void DashBoard::setFileOpened()
{
    emit fileOpened();
}
...
void DashBoard::setrpmstyle4(const int &rpmstyle4)
{
    if (m_rpmstyle4 == rpmstyle4)
        return;
    m_rpmstyle4 = rpmstyle4;
    emit rpmstyle4Changed(rpmstyle4);
}
...
QString DashBoard::newDashFile() const { return m_newdashfile; }

// Odometer
qreal DashBoard::Odo() const { return m_Odo; }
qreal DashBoard::orient() const { return m_Orient; }
...
QStringList DashBoard::dashsetup5() const { return m_dashsetup5; }
QStringList DashBoard::dashsetup4() const { return m_dashsetup4; }
...
qreal DashBoard::pageNo() const { return m_pageNo; }
qreal DashBoard::sourceIndex() const { return m_sourceIndex; }
...
int DashBoard::rpmstyle4() const { return m_rpmstyle4; }
