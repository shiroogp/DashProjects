...
Receiver::Receiver(DashBoard *dashboard, const Device& sender, QTcpSocket* socket, QObject* parent)
    : Transfer(socket, parent),mdashboard(dashboard), mSenderDev(sender), mFileSize(0), mBytesRead(0)
{
...
void Receiver::onDisconnected()
{
    ...
    emit mdashboard->errorOcurred("Sender disconnected");
}
...
void Receiver::processHeaderPacket(QByteArray& data)
{
    ...
    QString dstFolderPath = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)+"/dashfiles/";//Settings::instance()->getDownloadDir();
    ...
    if (mFile->open(QIODevice::WriteOnly)) {
        ...
        mdashboard->setFileOpened();
    }
    else {
        emit mInfo->errorOcurred(tr("Failed to write ") + dstFilePath);
        mdashboard->errorOcurred(tr("Failed to write ") + dstFilePath);
    }
}
