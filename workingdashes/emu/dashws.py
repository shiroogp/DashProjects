...
while loop:
    try:
        ...
        speed_kph=  int(rpm / 10)  #float(list[102])
        speed=  speed_kph / 1.6
        coolantRaw = int(list[7])
        coolant = coolantRaw - 40
        iatRaw = int(list[6])
        intake = iatRaw - 40
        load = int(list[10])
        throttle = int(list[24])
        if "rpm" in units:
            values["rpm"] = int(float(list[14]))
        if "tps" in units:
            values["tps"] = int(list[24])
        if "iat" in units:
            values["iat"] = int(list[6])
        if "map" in units:
            values["map"] = int(list[4])
        if "inj_pw" in units:
            values["inj_pw"] = int(list[75])
        speed = speed_kph
            # Odometer
        speed_sum += speed
        speed_sum_counter += 1
        if speed_sum_counter >= 100:
            ...
            speed_sum_counter = 0
        if "oil_t" in units:
            values["oil_t"] = int(list[114])
        if "oil_p" in units:
            values["oil_p"] = int(list[106])
        if "fuel_p" in units:
            values["fuel_p"] = int(list[105])
        if "clt_t" in units:
            values["clt_t"] = int(list[7])
        if "ign_ang" in units:
            values["ign_ang"] = int(list[116])
        if "dwell" in units:
            values["dwell"] = int(list[89])
            # if "lambda" in units:
