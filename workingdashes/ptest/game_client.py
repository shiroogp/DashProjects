...
class SerialThread(threading.Thread):
    def __init__(self, port, baudrate, databit, stopbit, parity, ws):
        ...
        self.q_text_pos.centerx = self.screen.get_rect().centerx + (100*self.xfactor)
        self.q_text_pos.centery = self.screen.get_rect().centery + (100*self.yfactor)
        ...
        self.temperature = 55
        self.message = ""
        self.oldspeed = 55 # temp
        self.oldspeed_kph=  48

        self.oldrpm = 879 # temp
        self.oldcoolant = 170
        self.oldintake = 110
        self.oldload = 44
        self.oldthrottle = 78  
        print("Client variables initialized")
        ...
        try:
            while not self.stop_event.is_set():
                ...
                try:
                    ...
                    # print(result)
                    list = result.split()
                    self.rpm = float(list[14])
                    self.speed_kph=  self.rpm / 10
                    self.speed=  self.speed_kph / 1.6
                    self.coolantRaw = float(list[7])
                    self.coolant = self.coolantRaw - 40
                    self.iatRaw = float(list[6])
                    self.intake = self.iatRaw - 40
                    self.load = list[10]
                    self.throttle = list[24]

                    self.oldspeed = self.speed # temp
                    self.oldspeed_kph=  self.speed_kph

                    self.oldrpm = self.rpm # temp
                    self.oldcoolant = self.coolant
                    self.oldintake = self.intake
                    self.oldload = self.load
                    ...
                    self.speed = self.oldspeed # temp
                    self.dspeed_kph=  self.oldspeed_kph

                    self.rpm = self.oldrpm # temp
                    self.coolant = self.oldcoolant
                    self.intake = self.oldintake
                    self.load = self.oldload
                    self.throttle = self.oldthrottle
                    # temperature = getTMP()
                # #speed = getSpeed()
                ...
                if self.display_compass:
                    ...
                    self.screen.blit(self.temp_text, self.t_text_pos) 
                    ...
                    if self.display_kph:
                        self.velocity_text = self.font3.render(str(self.speed_kph)+' km/h', 1, self.blue)
                    else:
                        self.velocity_text = self.font3.render(str(self.speed)+' mph', 1, self.blue)
                    self.screen.blit(self.velocity_text, self.v_text_pos)
                    ...
                    self.rpm_text = self.font3.render(str(self.rpm)+' rpm', 1, self.blue)
                    ...
                    self.ctemp_text = self.font2.render(str(self.coolant)+'\xb0F', 1, self.blue)
                    self.screen.blit(self.ctemp_text, self.ctemp_pos)
                    
                    self.itemp_text = self.font2.render(str(self.intake)+'\xb0F', 1, self.blue)
                    self.screen.blit(self.itemp_text, self.itemp_pos)

                    self.lvalue_text = self.font2.render(str(self.load)+'%', 1, self.blue)
                    self.screen.blit(self.lvalue_text, self.lvalue_text_pos)
                    
                    self.tvalue_text = self.font2.render(str(self.throttle)+'%', 1, self.blue)
