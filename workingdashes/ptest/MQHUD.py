...
def message_handling(client, userdata, msg):
    # print("Message: "+msg)
    global qmessage
    qmessage = msg.payload.decode()
    global time_count,rpm,speed,speed_kph,coolant,coolantRaw,iatRaw,intake,load,throttle,display_compass,display_F,display_kph,screen,black,white,blue,red,width, height,xfactor,yfactor,compass_background,compass_needle,cb_rect ,cn_rect ,font,font2,font3,font_small ,quit_text,q_text_pos,compass_text ,c_text_pos ,degree_text ,d_text_pos ,temp_text ,t_text_pos ,accel_text ,a_text_pos ,velocity_text ,v_text_pos ,rpm_text ,r_text_pos ,dir_text ,dir_text_pos ,coolant_text ,cool_text_pos ,ctemp_text ,ctemp_pos ,intake_text ,intake_text_pos ,itemp_text ,itemp_pos ,ambient_text ,ambient_text_pos ,load_text ,load_text_pos ,lvalue_text ,lvalue_text_pos ,tvalue_text ,tvalue_text_pos ,throttle_text ,throttle_text_pos ,acceleration_text ,acceleration_text_pos ,speed_text ,speed_text_pos ,rotation_text ,rotation_text_pos ,direction_text ,direction_text_pos ,display_compass ,display_F ,display_kph ,time_count,acceleration,temperature

    time_count += 1
    try:
        result =  qmessage    
        print("MQ message: "+result)
        # print("Message: "+msg)
        # qmessage = msg.payload.decode()
        # print("qMessage: "+qmessage)
        list = result.split()
        rpm = float(list[14])
        speed_kph=  rpm / 10
        speed=  speed_kph / 1.6
    ...
    for event in pygame.event.get():
      if event.type == pygame.MOUSEBUTTONDOWN:
        ...
        print("Mouse click: " + str(p[0]) + ":" + str(p[1]))
  ...
  #   print ("acceleration is " + str(acceleration))
    ...
    if display_compass:
      ...
      pygame.draw.line(screen, white, (20*xfactor, 70*yfactor), (300*xfactor, 70*yfactor))  
      pygame.draw.line(screen, white, (20*xfactor, 140*yfactor), (300*xfactor, 140*yfactor))  
      pygame.draw.line(screen, white, (width/3, 20*yfactor), (width/3, 200*yfactor))  
      pygame.draw.line(screen, white, (2*width/3, 20*yfactor), (2*width/3, 200*yfactor))  
    ...
    pygame.display.flip()    # print(f"{msg.topic}: {msg.payload.decode()}")
...
compass_background = pygame.transform.scale(compass_background, (int(200*xfactor),int(200*yfactor)))
compass_needle = pygame.image.load('compass_needle.png')
compass_needle = pygame.transform.scale(compass_needle, (int(150*xfactor),int(150*yfactor)))
cb_rect = compass_background.get_rect()
cn_rect = compass_background.get_rect()

cb_rect.centerx = 165*xfactor
cb_rect.centery = 107*yfactor
cn_rect.centerx = 193*xfactor
cn_rect.centery = 130*yfactor

font = pygame.font.Font(None, int(20*yfactor))
font2 = pygame.font.Font(None, int(30*yfactor))
font3 = pygame.font.Font(None, int(27*yfactor))
font_small = pygame.font.Font(None, int(18*yfactor))

quit_text = font.render("QUIT",1,(255,250,255))   #set up texts as buttons
q_text_pos = quit_text.get_rect()
q_text_pos.centerx = screen.get_rect().centerx + (100*xfactor)
q_text_pos.centery = screen.get_rect().centery + (100*yfactor)

compass_text = font.render("COMPASS",1,(255,250,255))
c_text_pos = compass_text.get_rect()
c_text_pos.centerx = 70*xfactor
c_text_pos.centery = 220*yfactor

degree_text = font2.render("degrees",1,(255,250,255))
d_text_pos = degree_text.get_rect()
d_text_pos.centerx = 295*xfactor
d_text_pos.centery = 25*yfactor

temp_text = font2.render("temp",1,(255,250,255))
t_text_pos = temp_text.get_rect()
t_text_pos.centerx = 265*xfactor
t_text_pos.centery = 180*yfactor

accel_text = font3.render("accel",1,(255,250,255))
a_text_pos = accel_text.get_rect()
a_text_pos.centerx = 255*xfactor
a_text_pos.centery = 110*yfactor

velocity_text = font3.render("velocity",1,(255,250,255))
v_text_pos = velocity_text.get_rect()
v_text_pos.centerx = 60*xfactor
v_text_pos.centery = 45*yfactor

rpm_text = font3.render("rpm",1,(255,250,255))
r_text_pos = velocity_text.get_rect()
r_text_pos.centerx = 165*xfactor
r_text_pos.centery = 45*yfactor

dir_text = font3.render("N",1,(255,250,255))
dir_text_pos = velocity_text.get_rect()
dir_text_pos.centerx = 290*xfactor
dir_text_pos.centery = 45*yfactor

coolant_text = font_small.render("Coolant:",1,white)
cool_text_pos = coolant_text.get_rect()
cool_text_pos.centerx = 60*xfactor
cool_text_pos.centery = 156 *yfactor

ctemp_text = font2.render("123", 1, white)
ctemp_pos = ctemp_text.get_rect()
ctemp_pos.centerx = 45*xfactor
ctemp_pos.centery = 180*yfactor

intake_text = font_small.render("Intake:",1,white)
intake_text_pos = coolant_text.get_rect()
intake_text_pos.centerx = 165*xfactor
intake_text_pos.centery = 156*yfactor

itemp_text = font2.render("123", 1, white)
itemp_pos = ctemp_text.get_rect()
itemp_pos.centerx = 150*xfactor
itemp_pos.centery = 180*yfactor

ambient_text = font_small.render("Ambient:",1,white)
ambient_text_pos = coolant_text.get_rect()
ambient_text_pos.centerx = 260*xfactor
ambient_text_pos.centery = 156*yfactor

load_text = font_small.render("Load:",1,white)
load_text_pos = load_text.get_rect()
load_text_pos.centerx = 60*xfactor
load_text_pos.centery = 85*yfactor

lvalue_text = font3.render("55",1, blue)
lvalue_text_pos = lvalue_text.get_rect()
lvalue_text_pos.centerx = 50*xfactor
lvalue_text_pos.centery = 110*yfactor

tvalue_text = font3.render("55",1, blue)
tvalue_text_pos = tvalue_text.get_rect()
tvalue_text_pos.centerx = 160*xfactor
tvalue_text_pos.centery = 110*yfactor

throttle_text = font_small.render("Throttle:",1,white)
throttle_text_pos = throttle_text.get_rect()
throttle_text_pos.centerx = 165*xfactor
throttle_text_pos.centery = 85*yfactor

acceleration_text = font_small.render("Acceleration:",1,white)
acceleration_text_pos = acceleration_text.get_rect()
acceleration_text_pos.centerx = 265*xfactor
acceleration_text_pos.centery = 85*yfactor

speed_text = font_small.render("Speed:",1,white)
speed_text_pos = speed_text.get_rect()
speed_text_pos.centerx = 60*xfactor
speed_text_pos.centery = 16*yfactor

rotation_text = font_small.render("RPM:",1,white)
rotation_text_pos = rotation_text.get_rect()
rotation_text_pos.centerx = 165*xfactor
rotation_text_pos.centery = 16*yfactor

direction_text = font_small.render("Direction:",1,white)
direction_text_pos = direction_text.get_rect()
direction_text_pos.centerx = 265*xfactor
direction_text_pos.centery = 16*yfactor
...
try:
    ...
    print("Caught an Exception, something went wrong..." + e)
finally:
