...
class PGPExampleUtil {
    ...
    static PGPPublicKey readPublicKey(String str) throws IOException, PGPException {
        ...
        PGPPublicKey readPublicKey = readPublicKey((InputStream) bufferedInputStream);
        bufferedInputStream.close();
        return readPublicKey;
    }

    static PGPPublicKey readPublicKey(InputStream inputStream) throws IOException, PGPException {
        Iterator<PGPPublicKeyRing> keyRings = new PGPPublicKeyRingCollection(PGPUtil.getDecoderStream(inputStream), (KeyFingerPrintCalculator) new JcaKeyFingerprintCalculator()).getKeyRings();
    ...
    static PGPSecretKey readSecretKey(String str) throws IOException, PGPException {
        BufferedInputStream bufferedInputStream = new BufferedInputStream(new FileInputStream(str));
        PGPSecretKey readSecretKey = readSecretKey((InputStream) bufferedInputStream);
        bufferedInputStream.close();
        return readSecretKey;
    }

    static PGPSecretKey readSecretKey(InputStream inputStream) throws IOException, PGPException {
        Iterator<PGPSecretKeyRing> keyRings = new PGPSecretKeyRingCollection(PGPUtil.getDecoderStream(inputStream), (KeyFingerPrintCalculator) new JcaKeyFingerprintCalculator()).getKeyRings();
        while (keyRings.hasNext()) {
