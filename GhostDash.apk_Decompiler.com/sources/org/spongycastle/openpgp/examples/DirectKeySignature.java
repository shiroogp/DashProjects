...
public class DirectKeySignature {
    public static void main(String[] strArr) throws Exception {
        ...
        if (strArr.length == 1) {
            Iterator signaturesOfType = new PGPPublicKeyRing(PGPUtil.getDecoderStream(new FileInputStream(strArr[0])), (KeyFingerPrintCalculator) new JcaKeyFingerprintCalculator()).getPublicKey().getSignaturesOfType(31);
            ...
            PGPSecretKeyRing pGPSecretKeyRing = new PGPSecretKeyRing(PGPUtil.getDecoderStream(new FileInputStream(strArr[0])), (KeyFingerPrintCalculator) new JcaKeyFingerprintCalculator());
            PGPPublicKeyRing pGPPublicKeyRing = new PGPPublicKeyRing((InputStream) new ByteArrayInputStream(signPublicKey(pGPSecretKeyRing.getSecretKey(), strArr[1], new PGPPublicKeyRing(PGPUtil.getDecoderStream(new FileInputStream(strArr[2])), (KeyFingerPrintCalculator) new JcaKeyFingerprintCalculator()).getPublicKey(), strArr[3], strArr[4])), (KeyFingerPrintCalculator) new JcaKeyFingerprintCalculator());
