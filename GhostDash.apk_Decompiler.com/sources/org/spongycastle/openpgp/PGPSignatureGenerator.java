...
public class PGPSignatureGenerator {
    ...
    public PGPSignature generate() throws PGPException {
        ...
        if (!packetPresent(this.hashed, 2)) {
            signatureSubpacketArr = insertSubpacket(this.hashed, new SignatureCreationTime(false, new Date()));
        } else {
            signatureSubpacketArr = this.hashed;
        }
        SignatureSubpacket[] signatureSubpacketArr3 = signatureSubpacketArr;
        if (packetPresent(this.hashed, 16) || packetPresent(this.unhashed, 16)) {
            signatureSubpacketArr2 = this.unhashed;
        } else {
            signatureSubpacketArr2 = insertSubpacket(this.unhashed, new IssuerKeyID(false, this.contentSigner.getKeyID()));
        }
        SignatureSubpacket[] signatureSubpacketArr4 = signatureSubpacketArr2;
        byte b = (byte) 4;
        try {
            ...
            for (int i = 0; i != signatureSubpacketArr3.length; i++) {
                signatureSubpacketArr3[i].encode(byteArrayOutputStream2);
            }
            ...
            return new PGPSignature(new SignaturePacket(this.sigType, this.contentSigner.getKeyID(), this.contentSigner.getKeyAlgorithm(), this.contentSigner.getHashAlgorithm(), signatureSubpacketArr3, signatureSubpacketArr4, new byte[]{digest[0], digest[1]}, dsaSigToMpi));
        } catch (IOException e) {
