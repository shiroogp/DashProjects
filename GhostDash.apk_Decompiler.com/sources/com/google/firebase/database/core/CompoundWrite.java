...
public final class CompoundWrite implements Iterable<Map.Entry<Path, Node>> {
    ...
    public List<NamedNode> getCompleteChildren() {
        ...
        if (this.writeTree.getValue() != null) {
            ...
            Iterator<Map.Entry<ChildKey, ImmutableTree<Node>>> it2 = this.writeTree.getChildren().iterator();
            while (it2.hasNext()) {
    ...
    private Node applySubtreeWrite(Path path, ImmutableTree<Node> immutableTree, Node node) {
        ...
        Node node2 = null;
        Iterator<Map.Entry<ChildKey, ImmutableTree<Node>>> it2 = immutableTree.getChildren().iterator();
        while (it2.hasNext()) {
            Map.Entry next = it2.next();
