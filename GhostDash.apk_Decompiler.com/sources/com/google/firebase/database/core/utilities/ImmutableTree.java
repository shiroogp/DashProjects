...
public final class ImmutableTree<T> implements Iterable<Map.Entry<Path, T>> {
    ...
    public T rootMostValueMatching(Path path, Predicate<? super T> predicate) {
        ...
        while (it2.hasNext() && (immutableTree = immutableTree.children.get(it2.next())) != null) {
    ...
    public boolean containsMatchingValue(Predicate<? super T> predicate) {
        ...
        Iterator<Map.Entry<ChildKey, ImmutableTree<T>>> it2 = this.children.iterator();
        while (it2.hasNext()) {
            if (((ImmutableTree) it2.next().getValue()).containsMatchingValue(predicate)) {
                return true;
            }
        }
        return false;
