...
public class WriteTree {
    ...
    public boolean removeWrite(long j) {
        ...
        Iterator<UserWriteRecord> it2 = this.allWrites.iterator();
        int i = 0;
        while (true) {
            if (!it2.hasNext()) {
        ...
        if (userWriteRecord.isOverwrite()) {
            ...
            Iterator<Map.Entry<Path, Node>> it3 = userWriteRecord.getMerge().iterator();
            while (it3.hasNext()) {
                this.visibleWrites = this.visibleWrites.removeWrite(userWriteRecord.getPath().child((Path) it3.next().getKey()));
            }
        }
        return true;
    }
    ...
    private boolean recordContainsPath(UserWriteRecord userWriteRecord, Path path) {
        ...
        while (it2.hasNext()) {
            if (userWriteRecord.getPath().child((Path) it2.next().getKey()).contains(path)) {
                return true;
            }
        }
        return false;
