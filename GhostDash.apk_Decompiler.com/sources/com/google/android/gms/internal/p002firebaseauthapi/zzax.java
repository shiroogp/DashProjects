...
public final class zzax {
    ...
    private static int zzh() {
        ...
        while (b == 0) {
            ...
            b = ((bArr[0] & ByteCompanionObject.MAX_VALUE) << 24) | ((bArr[1] & UByte.MAX_VALUE) << Tnaf.POW_2_WIDTH) | ((bArr[2] & UByte.MAX_VALUE) << 8) | (bArr[3] & UByte.MAX_VALUE);
        }
        return b;
    }
    ...
    private final synchronized boolean zzk(int i) {
        boolean z;
        Iterator<zziq> it2 = this.zza.zze().iterator();
        while (true) {
            if (it2.hasNext()) {
                if (it2.next().zza() == i) {
                    z = true;
