...
class RNFetchBlobConfig {
    ...
    RNFetchBlobConfig(ReadableMap readableMap) {
        ...
        if (readableMap != null) {
            ...
            this.mime = readableMap.hasKey(CMSAttributeTableGenerator.CONTENT_TYPE) ? readableMap.getString(CMSAttributeTableGenerator.CONTENT_TYPE) : str;
            this.increment = Boolean.valueOf(readableMap.hasKey(ServerValues.NAME_OP_INCREMENT) ? readableMap.getBoolean(ServerValues.NAME_OP_INCREMENT) : false);
            this.auto = Boolean.valueOf(readableMap.hasKey("auto") ? readableMap.getBoolean("auto") : z);
            if (readableMap.hasKey(FetchErrorStrings.CONNECTION_TIMEOUT)) {
                this.timeout = (long) readableMap.getInt(FetchErrorStrings.CONNECTION_TIMEOUT);
            }
        }
    }
}
