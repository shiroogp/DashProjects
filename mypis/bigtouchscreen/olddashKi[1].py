...
while (connected == False):
   try:
      ...
      ArduinoSerial = serial.Serial('/dev/ttyACM0', 115200, timeout=0.012)
      print("ECU Ready")
      connected = True
      time.sleep(3)
   except:
      try:
         # timeout for response on readline()
         ArduinoSerial = serial.Serial('/dev/ttyUSB0', 115200, timeout=0.012)
         print("ECU Ready")
         connected = True
         time.sleep(3)
      except:
...
while True:
    ...
    try:
	    ...
	    if (temprpm < 0):
	        temprpm = temprpm * -1
	    vars[14] = str(temprpm)
	    # basicdashdata = "BASIC~"+vars[4]+" "+vars[40]+" "+vars[7]+" "+vars[6]+" "+vars[114] + \
	    #     " "+vars[10]+" "+vars[105]+" "+vars[106] + \
	    #	     " "+vars[14]+" "+vars[9]+" "+vars[24]
