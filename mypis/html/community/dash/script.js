...
const linearRPM = () => {
  ...
  for (let index = rpmM; index >= 0; index--) {
    ...
    numHolder.setAttribute('style', `animation-delay: ${((index+15)*0.2)}s; transform: translateX(${(index/rpmM/1.28) * -180}px);`)
...
const linearKMH = () => {
  ...
  for (let index = (items.length - 1); index >= 0; index--) {
    let numHolder = document.createElement("div")
    numHolder.setAttribute('style', `animation-delay: ${((index+15)*0.2)}s; transform: translateX(${(index/(items.length - 1)/1.28) * 180}px);`)
